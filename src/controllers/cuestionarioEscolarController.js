...
// POST /api/cuestionario-escolar/mi-respuesta — estudiante, una sola vez
async function enviarMiRespuesta(req, res, next) {
  try {
    const yaExiste = await CuestionarioEscolar.findOne({
    ...
    const informacion = await CuestionarioEscolar.create({
      userId: req.usuario._id,
      respuestas,
      reflexiones,
    });
...
// GET /api/cuestionario-escolar/mi-respuesta — estudiante:
...
async function obtenerMiEstado(req, res, next) {
  try {
    const informacion = await CuestionarioEscolar.findOne({
      userId: req.usuario._id,
...
// GET /api/cuestionario-escolar/:userId — coordinadora/admin:
