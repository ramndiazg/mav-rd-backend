...
Estudiante — se registra gratis, paga en efectivo (confirmado manualmente por
coordinadora), accede al Aula Virtual solo tras pago confirmado.
Coordinadora — puede haber varias cuentas. Confirma pagos, desbloquea sesiones/
...
Admin / Fundadora — todo lo anterior + módulo de contabilidad completo
...
Registro: gratuito. Datos: nombre, apellido, cédula, teléfono, email, provincia,
fecha de nacimiento, contraseña. Sin foto.
Inscripción: dos planes con precios distintos — normal y vip. El pago es en
...
Sesiones: 3 sesiones, en orden estricto. Cada una tiene teoría + videos + examen.
...
Exámenes: 10 preguntas, banco de varias versiones por sesión (la coordinadora
...
Diploma: se emite un único diploma al completar y aprobar las 3 sesiones. Lo
...
Noticias: texto + imágenes + video embebido + botones de compartir (Facebook,
...
Contabilidad: categorías de gasto = sueldos, materiales, transporte, publicidad,
...
Sesión 7 — 05/07/2026 — Revisión de arquitectura: corrección de vacíos de lógica (sin código nuevo todavía)
...
La estudiante no podía recuperar el id de su IntentoExamen → nuevo
GET /api/intentos-examen/activo/:sesionId.
sesionActualDesbloqueada arranca en 0 y nada lo subía a 1 al pagar →
ahora es regla formal: confirmar-pago debe inicializarlo en 1. Falta
verificar en el código real si esto ya se hace o hay que implementarlo.
La estudiante no podía ver su propio estado de pago → nuevo
GET /api/inscripciones/me.
No estaba definido quién elige la versión de examen al desbloquear →
...
configuracion, que ya se usaba para los precios).
Arquitectura_Frontend.md tenía una sección vieja contradictoria que
...
Nota menor: el código de verificación de diploma debe usar el año actual
dinámicamente (new Date().getFullYear()), no 2026 hardcodeado — revisar
verificationCode.js real y corregir si sigue fijo.

No se tocó código todavía — esta sesión fue solo de diseño/documentación.
El próximo bloque de trabajo debe: (a) implementar los 3 endpoints nuevos y
los 2 cambios de comportamiento (desbloquear por sesión + azar, e
inicialización de sesionActualDesbloqueada) en el backend real, (b) verificar
si sesionActualDesbloqueada ya se inicializaba en 1 o si es un bug real, y
(c) recién después seguir con el Aula Virtual del frontend, ya con el backend
alineado a lo documentado aquí.

Pendiente para la próxima sesión:

Implementar en el backend real los cambios listados arriba (puntos 1-7 y 9)
Confirmar/corregir la inicialización de sesionActualDesbloqueada
Reemplazar exámenes de prueba por preguntas reales una vez el banco
soporte PATCH/DELETE
